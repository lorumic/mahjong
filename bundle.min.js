...
addFooter(panel,modalLabel="OK",resolve=()=>{},botDismissible){let ok=document.createElement("button");ok.id="okButton";ok.textContent=modalLabel;ok.addEventListener("click",()=>{this.close([{object:document,evntName:"focus",handler:panel.gainFocus},]);resolve()});panel.appendChild(ok);if(config.BOT_PLAY&&botDismissible){let dismiss=()=>ok.click();setTimeout(()=>dismiss(),config.HAND_INTERVAL);panel.addEventListener("click",()=>(dismiss=()=>{}))}
...
pickPlaySettings(){this.reveal();this.settings=new SettingsModal(this);this.settings.show()}
...
async preparePlay(redraw){await this.continue("preparePlay");this.currentPlayerId=(this.wind%4);this.discard=undefined;this.counter=0;let players=this.players;await Promise.all(players.map(p=>{return new Promise(ready=>p.handWillStart(redraw,ready))}));if(!noSleepEnabled){var noSleep=new NoSleep();noSleep.enable();noSleepEnabled=!0}
await Promise.all(players.map(p=>{return new Promise(done=>this.resolveKongs(p,done))}))}
...
this.enabled=!1}},{key:"isEnabled",get:function get(){return this.enabled}}]);return NoSleep}();var noSleepEnabled=!1;(function(){if(config.PLAY_IMMEDIATELY)play();else offerChoice();function play(){let manager=new GameManager();let game=manager.newGame();game.startGame(()=>{document.body.classList.add("finished");let gameui=game.players.find((p)=>p.ui).ui;config.flushLog();return modal.showFinalScores(gameui,game.rules,game.scoreHistory,()=>{document.body.classList.remove("finished");rotateWinds.reset();offerChoice()})})}

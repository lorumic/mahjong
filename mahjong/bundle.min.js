...
game.discard=new GameTile(null,game.discard.values);game.discards.push(game.discard);for(var i=0;i<game.discards.length;i++){let tile=game.discards[i].values.tile;let discard=create(tile);discard.mark(`discard`);document.querySelector(`.discards`).appendChild(discard);if(i===game.discards.length-1){game.discard=discard}

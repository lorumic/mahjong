(function(){'use strict';let max=Math.max;let random=v=>v?(Math.random()*v)|0:Math.random();function Random(seed){this._seed=seed?seed%2147483647:random(2147483647);console.log(`using initial random seed ${this._seed}`);if(this._seed<=0)this._seed+=2147483646}
...
Ruleset.register(ChineseClassical);const noop=()=>{};const __console_debug=console.debug.bind(console);const DEFAULT_CONFIG={DEBUG:!1,USE_SOUND:!0,SEED:0,RULES:`Chinese Classical`,PLAY_IMMEDIATELY:!1,PAUSE_ON_BLUR:!0,FORCE_DRAW:!1,FORCE_OPEN_BOT_PLAY:!1,SHOW_CLAIM_SUGGESTION:!0,SHOW_BOT_SUGGESTION:!0,BOT_CHICKEN_THRESHOLD:0.0008,CLAIM_INTERVAL:5000,PLAY_INTERVAL:100,HAND_INTERVAL:3000,BOT_DELAY_BEFORE_DISCARD_ENDS:300,BOT_PLAY_DELAY:50,WALL_HACK:"",WRITE_GAME_LOG:!1,};const fromStorage=JSON.parse(localStorage.getItem("mahjongConfig")||"{}");let currentConfig=Object.assign(fromStorage,DEFAULT_CONFIG);let queryArgs=globalThis.location.search.replace(/([a-z_]+)=/g,(a,b)=>b.toUpperCase()+"=");let params=new URLSearchParams(queryArgs);let urlConfig=Object.fromEntries(params.entries());currentConfig=Object.assign(currentConfig,urlConfig);for(const[key,value]of Object.entries(currentConfig)){if(value==="true")currentConfig[key]=!0;if(value==="false")currentConfig[key]=!1;if(value==parseFloat(value))currentConfig[key]=parseFloat(value);}
console.log("playing with",currentConfig);if(currentConfig.WALL_HACK||currentConfig.PLAY_IMMEDIATELY){currentConfig.FORCE_OPEN_BOT_PLAY=!0;currentConfig.USE_SOUND=!0}
const CLAIM$1={IGNORE:0,PAIR:1,CHOW:2,CHOW1:4,CHOW2:5,CHOW3:6,PUNG:8,KONG:16,SET:30,WIN:32,};const Constants={PAIR:CLAIM$1.PAIR,CHOW:CLAIM$1.CHOW,CHOW1:CLAIM$1.CHOW1,CHOW2:CLAIM$1.CHOW2,CHOW3:CLAIM$1.CHOW3,PUNG:CLAIM$1.PUNG,KONG:CLAIM$1.KONG,SET:CLAIM$1.SET,WIN:CLAIM$1.WIN,};const TILE_NAMES={0:"bamboo 1",1:"bamboo 2",2:"bamboo 3",3:"bamboo 4",4:"bamboo 5",5:"bamboo 6",6:"bamboo 7",7:"bamboo 8",8:"bamboo 9",9:"characters 1",10:"characters 2",11:"characters 3",12:"characters 4",13:"characters 5",14:"characters 6",15:"characters 7",16:"characters 8",17:"characters 9",18:"dots 1",19:"dots 2",20:"dots 3",21:"dots 4",22:"dots 5",23:"dots 6",24:"dots 7",25:"dots 8",26:"dots 9",27:"east",28:"south",29:"west",30:"north",31:"green dragon",32:"red dragon",33:"white dragon",34:"flower 1",35:"flower 2",36:"flower 3",37:"flower 4",38:"season 1",39:"season 2",40:"season 3",41:"season 4",};const TILE_GLYPHS={0:"b1",1:"b2",2:"b3",3:"b4",4:"b5",5:"b6",6:"b7",7:"b8",8:"b9",9:"c1",10:"c2",11:"c3",12:"c4",13:"c5",14:"c6",15:"c7",16:"c8",17:"c9",18:"d1",19:"d2",20:"d3",21:"d4",22:"d5",23:"d6",24:"d7",25:"d8",26:"d9",27:"E",28:"S",29:"W",30:"N",31:"F",32:"C",33:"P",34:"f1",35:"f2",36:"f3",37:"f4",38:"s1",39:"s2",40:"s3",41:"s4",};const SUIT_NAMES={0:"bamboo",1:"characters",2:"dots",3:"winds",4:"dragons",5:"bonus",};const config=Object.assign({set:(opt)=>{Object.keys(opt).forEach((key)=>{let value=opt[key];if(typeof config[key]!=="undefined"){config[key]=value;if(key===`DEBUG`){if(value){console.log("activating");console.debug=__console_debug}else{console.log("deactivating");console.debug=noop}}}})},DEFAULT_CONFIG,PRNG:new Random(currentConfig.SEED),DEBUG:currentConfig.DEBUG,log:noop,flushLog:noop,START_OVERRIDE_SEED:0,START_ON_HAND:0,PAUSE_ON_HAND:0,START_ON_DRAWS:0,PAUSE_ON_DRAW:0,PAUSE_ON_PLAY:0,BOT_PLAY:!1,ARTIFICIAL_BOT_DELAY:300,LOSERS_SETTLE_SCORES:!0,CLAIM:CLAIM$1,Constants,TILE_NAMES,TILE_GLYPHS,SUIT_NAMES,convertSubtypeToClaim:(diff)=>{if(diff===-1)return CLAIM$1.CHOW3;if(diff===1)return CLAIM$1.CHOW2;if(diff===2)return CLAIM$1.CHOW1;return diff},},currentConfig);config.set({DEBUG:currentConfig.DEBUG});config.log=playlog.log;config.flushLog=playlog.flush;class GameTile extends HTMLElement{constructor(tile){super();tile=tile??this.getAttribute(`tile`)??-1;this.values={tile};this.setAttribute(`tile`,this.values.tile);this.setAttribute(`title`,TILE_NAMES[tile]);this.setAttribute(`alt`,TILE_NAMES[tile])}
...
bonus(){this.setAttribute(`bonus`,!0);this.lock()}
...
addFormControls(panel,form,options){const table=form.querySelector(`table`);let row=document.createElement(`tr`);row.classList.add(`spacer-1`);row.innerHTML=`
    ...
    `;table.appendChild(row);form.addEventListener(`submit`,(evt)=>{evt.preventDefault();let suffix=options.filter((e)=>e.value!=e.default_value).map((e)=>`${e.key}=${e.value}`).join("&");globalThis.location.search=suffix?`?${suffix}`:``});let ok=table.querySelector(`#ok`);panel.gainFocus=()=>ok.focus();let reset=table.querySelector(`#reset`);reset.addEventListener("click",(evt)=>(globalThis.location.search=""))}
getOptions(){const options=[{label:`Rules`,key:`rules`,options:[...Ruleset.getRulesetNames()],},{},{label:`🀄 Always show everyone's tiles`,key:`force_open_bot_play`,toggle:!0,},{label:`✨ Highlight claimable discards`,key:`show_claim_suggestion`,toggle:!0,},{label:`💬 Show bot play suggestions`,key:`show_bot_suggestion`,toggle:!0,},{},{label:`🎵 Play sounds`,key:`use_sound`,toggle:!0,},{label:`🟢 Start play immediately`,key:`play_immediately`,toggle:!0,},{label:`⏸️ Pause game unless focused`,key:`pause_on_blur`,toggle:!0,},{label:`💻 Turn on debug mode`,key:`debug`,toggle:!0,},{label:`❌ Pretend previous round was a draw`,key:`force_draw`,toggle:!0,debug_only:!0,},{label:`📃 Generate game log after play`,key:`write_game_log`,toggle:!0,debug_only:!0,},{},{label:`Set game PRNG seed`,key:`seed`,debug_only:!0,},{label:`Bot quick play threshold`,key:`bot_chicken_threshold`,debug_only:!0,},{label:`Delay (in ms) between player turns`,key:`play_interval`,},{label:`Delay (in ms) before starting next hand`,key:`hand_interval`,},{label:`Delay (in ms) for bots reacting to things`,key:`bot_delay_before_discard_ends`,},{label:`Delay (in ms) during full bot play`,key:`bot_play_delay`,},{label:`Set up a specific wall`,key:`wall_hack`,options:[``,...Object.keys(WallHack.hacks)],debug_only:!0,},];options.forEach((entry)=>{const{key}=entry;if(key){const CONFIG_KEY=key.toUpperCase();entry.value=config[CONFIG_KEY];entry.default_value=config.DEFAULT_CONFIG[CONFIG_KEY]}});return options}}
...
class TileSetManager{static loadDefault(){this.createTileSetCSS(`./img/tiles/default-tileset.svg`).then((css)=>setStyleSheet(`default-tiles`,css))}
static createTileSetCSS(dataURL){return new Promise((resolve)=>{const img=new Image();img.src=dataURL;img.onload=(evt)=>{const css=[];const tileWidth=img.width/9;const tileHeight=img.height/5;console.log(img.width,img.height,tileWidth,tileHeight);const canvas=document.createElement(`canvas`);canvas.width=img.width;canvas.height=img.height;const ctx=canvas.getContext(`2d`);ctx.drawImage(img,0,0,canvas.width,canvas.height);for(let r=0;r<5;r++){for(let c=0;c<9;c++){const tileNumber=TileSetManager.getTileNumber(r,c);if(tileNumber===!1)continue;const[x,y,w,h]=[tileWidth*c+1,tileHeight*r+1,tileWidth-2,tileHeight-2,];const crop=document.createElement(`canvas`);crop.width=w;crop.height=h;crop.getContext("2d").drawImage(canvas,x,y,w,h,0,0,w,h);css.push(`[tile="${tileNumber}"] { background-image: url(${crop.toDataURL()}); }`)}}
resolve(css.join(`\n`))}})}
static getTileNumber(row,col){if(row<3)return col+9*row;if(row===3){if(col<4)return 27+col;if(col===4)return!1;if(col<8)return 31-5+col}
if(row===4){if(col!==8)return 34+col;return-1}
return!1}}
function fileLoader(evt){return new Promise((resolve,reject)=>{const file=evt.target.files[0];if(file){const reader=new FileReader();reader.onload=(e)=>{file.dataURL=e.target.result;if(file.size>500000){alert("Images over 500kb are not supported");return reject()}
resolve(file)};reader.readAsDataURL(file)}else reject()})}
class ThemeModal{constructor(modal){this.modal=modal;this.init()}
init(){this.loadBackground();this.loadSidebar();this.loadPlayerBanks();this.loadTileset()}
reset(){[`mahjongBackground`,`mahjongSidebar`,`mahjongPlayerBanks`,`mahjongTileset`,`mahjongCSS`,].forEach((key)=>{localStorage.removeItem(key);const e=document.getElementById(`mahjongBackground`);if(e){e.parentNode.removeChild(e)}});globalThis.location.reload()}
loadBackground(){const dataURL=localStorage.getItem("mahjongBackground");if(dataURL){setStyleSheet(`mahjongBackground`,`.board .discards { background-image: url(${dataURL}); }`)}
return!!dataURL}
saveBackground(background){localStorage.setItem("mahjongBackground",background)}
loadSidebar(){const dataURL=localStorage.getItem("mahjongSidebar");if(dataURL){setStyleSheet(`mahjongSidebar`,`.board .sidebar { background-image: url(${dataURL}); }`)}
return!!dataURL}
saveSidebar(background){localStorage.setItem("mahjongSidebar",background)}
loadPlayerBanks(){const dataURL=localStorage.getItem("mahjongPlayerBanks");if(dataURL){setStyleSheet(`mahjongPlayerBanks`,`.players .player { background-image: url(${dataURL}); }`)}
return!!dataURL}
savePlayerBanks(background){localStorage.setItem("mahjongPlayerBanks",background)}
async loadTileset(){const dataURL=localStorage.getItem("mahjongTileset");if(dataURL){setStyleSheet(`mahjongTileset`,await TileSetManager.createTileSetCSS(dataURL))}else{TileSetManager.loadDefault()}
return!!dataURL}
saveTileset(background){localStorage.setItem("mahjongTileset",background)}
show(){const panel=this.modal.makePanel(`settings`);panel.innerHTML=`<h3>Change the game theme</h3>`;const options=this.getOptions();const table=this.modal.buildPanelContent(options);this.addFormControls(panel,table,options);this.modal.addFooter(panel,"Close")}
addFormControls(panel,table,options){let row=document.createElement(`tr`);row.classList.add(`spacer-1`);row.innerHTML=`
      <td colspan="2">
        <input id="reset" type="reset" value="Reset to default settings">
      </td>
    `;table.appendChild(row);let reset=table.querySelector(`#reset`);reset.addEventListener("click",()=>this.reset())}
getOptions(){const handle=(fnName)=>(entry,evt)=>fileLoader(evt).then((file)=>{this[`save${fnName}`](file.dataURL);this[`load${fnName}`]()});const options=[{label:"Background image",type:`file`,handler:handle("Background"),},{label:"Sidebar image",type:`file`,handler:handle("Sidebar"),},{label:"Player banks",type:`file`,handler:handle("PlayerBanks"),},{label:"Tileset",type:`file`,handler:handle("Tileset"),},{label:"CSS colors",button_label:"Change...",type:`button`,evtType:`click`,handler:(entry,evt)=>{this.modal.pickColors()}}];return options}}
class ColorModal{constructor(modal){this.modal=modal;this.overrides={};this.loadColorScheme()}
reset(){const style=document.querySelector(`style#mahjongCSS`);if(style)style.parentNode.removeChild(style);}
saveColor(entry){if(entry.value!==entry.default_value){this.overrides[entry.label]=entry}else{this.overrides[entry.label]=undefined}
const colorCSS=`:root {${Object.entries(this.overrides)
      .filter(([label, entry]) => !!entry)
      .map(([label, entry]) => `${entry.key}:${entry.value};`)
      .join(`\n`)}}`;localStorage.setItem(`mahjongCSS`,colorCSS);setStyleSheet(`mahjongCSS`,colorCSS)}
loadColorScheme(){const colorCSS=localStorage.getItem("mahjongCSS");if(colorCSS){setStyleSheet(`mahjongCSS`,colorCSS)}
return!!colorCSS}
show(){const panel=this.modal.makePanel(`settings`);panel.innerHTML=`<h3>Change CSS Colors</h3>`;const options=this.getOptions(panel);const table=this.modal.buildPanelContent(options);this.addFormControls(table);this.modal.addFooter(panel,"Close")}
addFormControls(table){let row=document.createElement(`tr`);row.classList.add(`spacer-1`);row.innerHTML=`
        <td colspan="2">
          <input id="reset" type="reset" value="Reset to default settings">
        </td>
      `;table.appendChild(row);let reset=table.querySelector(`#reset`);reset.addEventListener("click",()=>this.reset())}
getCSSColors(){const s=Array.from(document.styleSheets).find((s)=>s.ownerNode.href.includes(`/colors.css`));const colors=Array.from(s.rules[0].style);const colorsForHumans=colors.map((v)=>v.replace(/^--/,"").replaceAll("-"," "));const values=colors.map((v)=>getComputedStyle(document.documentElement).getPropertyValue(v));return{colors,colorsForHumans,values}}
getOptions(){const colors=this.getCSSColors();const get=(l)=>this.overrides[l]?.value;const hex=(c)=>this.getHexColor(c);const save=(entry)=>this.saveColor(entry);const options=colors.colorsForHumans.map((label,i)=>{return{label:label,key:colors.colors[i],value:get(label)||hex(colors.values[i]),default_value:hex(colors.values[i]),type:`color`,evtType:`input`,get handler(){return(entry,evt,opacity)=>{if(evt)this.value=hex(evt.target.value);if(opacity){this.value=this.value.substring(0,7)+parseInt(opacity).toString(16).padStart(2,"0")}
save(this)}},}});return options}
getHexColor(cssColor){const canvas=document.createElement(`canvas`);canvas.width=canvas.height=1;const ctx=canvas.getContext("2d");ctx.beginPath();ctx.rect(-1,-1,2,2);ctx.fillStyle=cssColor;ctx.fill();const[r,g,b,a]=Array.from(ctx.getImageData(0,0,1,1).data);if(a<255){const v=((r<<24)+(g<<16)+(b<<8)+a).toString(16);return `#${v.padStart(8, "0")}`}
const v=((r<<16)+(g<<8)+b).toString(16);return `#${v.padStart(6, "0")}`}}
class Modal{constructor(fixed=!1){this.fixed=fixed;this.modal=document.querySelector(".modal");this.gameBoard=document.querySelector(".board");this.panels=[];this.choice=new OptionsDialog(this);this.settings=new SettingsModal(this);this.theming=new ThemeModal(this);this.colors=new ColorModal(this);this.scores=new ScoreModal(this)}
...
addFooter(panel,modalLabel="OK",resolve=()=>{},botDismissible){let ok=document.createElement("button");ok.textContent=modalLabel;ok.addEventListener("click",()=>{this.close([{object:document,evntName:"focus",handler:panel.gainFocus},]);resolve()});panel.appendChild(ok);if(config.BOT_PLAY&&botDismissible){let dismiss=()=>ok.click();setTimeout(()=>dismiss(),config.HAND_INTERVAL);panel.addEventListener("click",()=>(dismiss=()=>{}))}
...
pickPlaySettings(){this.reveal();this.settings.show()}
pickTheming(){this.reveal();this.theming.show()}
pickColors(){this.reveal();this.colors.show()}}
let modal=new Modal();class TileTracker{constructor(id){this.id=id;this.tiles=[];this.ui=!1;this.reset()}
setUI(ui){this.ui=ui}
...
this.tiles[tileNumber]--;if(this.ui)this.ui.reduceTracker(tileNumber);}}
...
handWillStart(redraw,resolve){if(this.ui)this.ui.handWillStart(redraw,resolve);else resolve()}
...
tilesNeeded(){return tilesNeeded(this.getTileFaces(),this.locked)}
...
async getDiscard(tilesRemaining,resolve){let resolveProxy=(discard)=>{this.discards.push(discard);resolve(discard)};return this.determineDiscard(tilesRemaining,resolveProxy)}
...
if(config.PRNG.nextFloat()<this.chickenThreshold){this.chicken=!0;let notice=`player ${this.player.id} will be going for chicken hands at ${tilesRemaining} tiles left!`;console.log(notice);config.log(notice)}}
suit(t){return(t/9)|0}
...
let h=(wotr*4+wind);if(h===previous)return;previous=h;let p=(((h/4)|0)%4);let offset=(2*p);indicator.style.setProperty('--slide',offset+'em');if(rules.reverse_wind_direction){winds.forEach(e=>{if(e.classList.contains('tc')){e.classList.remove('tc');e.classList.add('lc')}else if(e.classList.contains('rc')){e.classList.remove('rc');e.classList.add('tc')}else if(e.classList.contains('bc')){e.classList.remove('bc');e.classList.add('rc')}else if(e.classList.contains('lc')){e.classList.remove('lc');e.classList.add('bc')}})}else{winds.forEach(e=>{if(e.classList.contains('tc')){e.classList.remove('tc');e.classList.add('rc')}else if(e.classList.contains('rc')){e.classList.remove('rc');e.classList.add('bc')}else if(e.classList.contains('bc')){e.classList.remove('bc');e.classList.add('lc')}else if(e.classList.contains('lc')){e.classList.remove('lc');e.classList.add('tc')}})}}
rotateWinds.reset=function(){previous=0;indicator.style.setProperty('--slide','0em');winds[0].setAttribute('class','player-wind tc e');winds[1].setAttribute('class','player-wind rc');winds[2].setAttribute('class','player-wind bc');winds[3].setAttribute('class','player-wind lc');indicator.classList.remove('done')};rotateWinds.done=function(){return(indicator.classList.add('done'))};return rotateWinds})();class ClientUIMaster{constructor(player,tracker){this.player=player;this.tracker=tracker;this.tracker.setUI(this);this.id=player.id;this.discards=document.querySelector(`.discards`);this.playerbanks=document.querySelectorAll(`.player`);this.knowledge=document.querySelector(`.knowledge`);this.settings=document.querySelector(`.settings`);this.theming=document.querySelector(`.theming`);this.gameBoard=document.querySelector(`.board`);if(config.PAUSE_ON_BLUR){this.gameBoard.addEventListener(`blur`,async(evt)=>{let resume=await this.player.game.pause();let handleResume=()=>{this.gameBoard.removeEventListener(`focus`,handleResume);resume();this.pause_protection=!0};this.gameBoard.addEventListener(`focus`,handleResume)})}
this.settings.addEventListener(`click`,()=>modal.pickPlaySettings());this.theming.addEventListener(`click`,()=>modal.pickTheming());this.el=this.playerbanks[this.id];this.reset(0,0);if(config.FORCE_OPEN_BOT_PLAY){globalThis.PLAYER_BANKS=this.playerbanks;globalThis.PLAYER_BANKS.sortTiles=e=>this.sortTiles(e)}}
...
reduceTracker(tileNumber){if(tileNumber>33)return;let tile=this.knowledge.querySelector(`game-tile[tile='${tileNumber}']`);tile.remove()}
...
startCountDown(ms){new TaskTimer(timer=>{this.countdownTimer=timer},()=>{this.countdownTimer=!1},ms,(count)=>{let fraction=count===10?1:count/10;this.bar.style.width=`${100 - 100 * fraction}%`;if(fraction===1){this.bar.classList.remove(`active`);this.countdownTimer=!1;if(this.claimCleanup)this.claimCleanup();this.claimCleanup=!1}},10);this.bar.classList.add(`active`)}
gameWillStart(){rotateWinds.reset();playClip(`start`);this.playerbanks.forEach(b=>{if(this.rules)b.dataset.score=this.rules.player_start_score;b.dataset.wins=0})}
...
removeLastDiscard(){if(this.discards.lastChild){this.discards.removeChild(this.discards.lastChild)}}
...
endOfHand(disclosure,force_reveal_player=!1){if(!disclosure){playClip(`draw`);this.discards.classList.add(`exhausted`);return}
...
recordScores(scores){scores.forEach((score,b)=>{let d=this.playerbanks[b].dataset;if(!d.score)d.score=0;d.score=parseInt(d.score)+score})}
...
remove(tile){this.el.removeChild(tile)}
...
let discard=create(tile);discard.mark(`discard`);discard.setFrom(player.id);this.discards.appendChild(discard);if(!config.BOT_PLAY&&player.id!==this.id){this.startCountDown(config.CLAIM_INTERVAL)}
this.sortTiles(bank)}
...
class HumanPlayer extends BotPlayer{constructor(id,chicken=!1){super(id,chicken);this.ui=new ClientUI(this,this.tracker)}
...
let base=[...new Array(34)].map((_,i)=>i);const BASE=base.concat(base).concat(base).concat(base).concat([34,35,36,37,38,39,40,41]);class Wall{constructor(players){this.players=players;this.reset()}
...
class Game{constructor(players){this.players=players;this.wall=new Wall(players);this.scoreHistory=[];this._playLock=!1;this.GAME_START=!1;this.resume=()=>{}}
async startGame(whenDone){document.body.classList.remove(`finished`);this.GAME_START=Date.now();this.currentpid=0;this.wind=0;this.windOfTheRound=0;this.hand=0;this.draws=0;this.totalDraws=0;this.totalPlays=0;this.finish=whenDone;this.rules=Ruleset.getRuleset(config.RULES);let players=this.players;await players.asyncAll(p=>p.gameWillStart(this,this.rules));this.fixValues=()=>{};config.log(`starting game.`);this.startHand()}
...
let revealed=!1;do{let tile=this.wall.get();config.log(`${player.id} <  ${tile} (supplement)`);revealed=player.append(tile);if(revealed){await players.asyncAll(p=>p.see(revealed,player))}}while(revealed);}
...
newGame(){let game=new Game(this.players);globalThis.currentGame={game:game,players:this.players};let gameBoard=document.querySelector('.board');gameBoard.focus();return game}}
...
this.enabled=!1}},{key:"isEnabled",get:function get(){return this.enabled}}]);return NoSleep}();var noSleepEnabled=!1;(function(){if(config.PLAY_IMMEDIATELY)play();else offerChoice();var noSleep=new NoSleep();function play(){if(!noSleepEnabled){noSleep.enable();noSleepEnabled=!0}
let manager=new GameManager();let game=manager.newGame();game.startGame(()=>{document.body.classList.add("finished");let gameui=game.players.find((p)=>p.ui).ui;config.flushLog();return modal.showFinalScores(gameui,game.rules,game.scoreHistory,()=>{document.body.classList.remove("finished");rotateWinds.reset();offerChoice()})})}
function offerChoice(){const options=[{description:"There are currently two modes of play on offer:"},{label:"I'd like to play some mahjong!",value:"play"},{label:"I just want to watch the bots play",value:"watch"},{description:"Alternatively, you can modify the game settings:",align:"center",},{label:"Change settings",value:"settings",back:!0},{label:"Change theming",value:"theming",back:!0},{description:"(you can also open the settings during play)",align:"center",},];options.fixed=!0;modal.choiceInput("Welcome! What would you like to do?",options,(result)=>{config.BOT_PLAY=result==="watch";if(result==="watch")config.FORCE_OPEN_BOT_PLAY=!0;if(result==="settings")return modal.pickPlaySettings();if(result==="theming")return modal.pickTheming();play()})}})()})()
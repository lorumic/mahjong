...
addFooter(panel,modalLabel="OK",resolve=()=>{},botDismissible){let ok=document.createElement("button");ok.id="okButton";ok.textContent=modalLabel;ok.addEventListener("click",()=>{this.close([{object:document,evntName:"focus",handler:panel.gainFocus},]);resolve();try{document.getElementById("okButton").click()}catch(_){}});panel.appendChild(ok);if(config.BOT_PLAY&&botDismissible){let dismiss=()=>ok.click();setTimeout(()=>dismiss(),config.HAND_INTERVAL);panel.addEventListener("click",()=>(dismiss=()=>{}))}
...
async getDiscard(tilesRemaining,resolve){let resolveProxy=(discard)=>{this.discards.push(discard);this.game.discards.push(discard);resolve(discard)};return this.determineDiscard(tilesRemaining,resolveProxy)}
...
startCountDown(ms){document.querySelector(".corner.settings").style.visibility="hidden";new TaskTimer(timer=>{this.countdownTimer=timer},()=>{this.countdownTimer=!1;if(globalThis.currentGame.game.currentPlayerId===3){document.querySelector(".corner.settings").style.visibility="visible"}},ms,(count)=>{let fraction=count===10?1:count/10;this.bar.style.width=`${100 - 100 * fraction}%`;if(fraction===1){this.bar.classList.remove(`active`);this.countdownTimer=!1;if(this.claimCleanup)this.claimCleanup();this.claimCleanup=!1;if(globalThis.currentGame.game.currentPlayerId===3){document.querySelector(".corner.settings").style.visibility="visible"}}},10);this.bar.classList.add(`active`)}
...
if(config.AUTO_SKIP_DISCARDS&&player.id!==this.id&&!canClaim){setTimeout(()=>document.querySelector(".discards").click(),500)}
this.sortTiles(bank)}
...
discardCurrentHighlightedTile(suggestions=[],resolve){let tiles=this.getAvailableTiles();this.cancelLongPress();suggestions.forEach(suggestion=>{if(suggestion){suggestion.unmark('suggestion');suggestion.setTitle('')}});let latest=this.player.latestTile;if(latest)latest.unmark('latest');tiles.forEach(tile=>tile.unmark('selectable','highlight','suggestion'));this.removeAllListeners();resolve(this.currentTile);let serializedGame=JSON.parse(this.serialize(globalThis.currentGame));serializedGame.game.discard={values:{tile:this.currentTile.getTileFace(),from:0}};serializedGame.game.rules=JSON.parse(JSON.stringify(globalThis.currentGame.game.rules));serializedGame.game.discards=[];for(let j=0;j<globalThis.currentGame.game.discards.length;j++){serializedGame.game.discards.push({values:globalThis.currentGame.game.discards[j].values})}
...
let playerTiles=globalThis.currentGame.players[i].tiles;serializedGame.game.players[i].tiles=[];for(let j=0;j<playerTiles.length;j++){var values=playerTiles[j].values;if(values.tile===serializedGame.game.discard.values.tile)continue;serializedGame.game.players[i].tiles.push({values:values})}
...
serializedGame.game.currentPlayerId=1;localStorage.setItem("mahjongGame",JSON.stringify(serializedGame))}
...
this.resume=()=>{}}
async startGame(whenDone,isResuming=!1){document.body.classList.remove(`finished`);this.finish=whenDone;this.fixValues=()=>{};if(isResuming){this.play()}else{this.GAME_START=Date.now();this.currentpid=0;this.wind=0;this.windOfTheRound=0;this.hand=0;this.draws=0;this.totalDraws=0;this.totalPlays=0;this.rules=Ruleset.getRuleset(config.RULES);let players=this.players;await players.asyncAll(p=>p.gameWillStart(this,this.rules));config.log(`starting game.`);this.startHand()}}
...
var gameObj=localStorageGame.game;this.players=[new HumanPlayer(0,!1,!1),new BotPlayer(1),new BotPlayer(2),new BotPlayer(3),];gameObj.rules=rules;gameObj.wall=new Wall(this.players,localStorageGame.game.wall);var game=new Game(this.players,gameObj);var lsPlayers=localStorageGame.game.players;var publiclyVisible=[];for(var i=0;i<4;i++){this.players[i].game=game;this.players[i].chicken=lsPlayers[i].chicken;this.players[i].discards=lsPlayers[i].discards;this.players[i].draws=lsPlayers[i].draws;this.players[i].el=document.getElementById(i);this.players[i].has_won=lsPlayers[i].has_won;this.players[i].lastClaim=lsPlayers[i].lastClaim;this.players[i].latest=lsPlayers[i].latest;var tileTracker=new TileTracker(i,lsPlayers[i].tracker.tiles);var ui=i===0?new ClientUI(this.players[i],tileTracker):!1;tileTracker.setUI(ui);if(ui)tileTracker.ui.resetTracker(tileTracker.tiles);this.players[i].tracker=tileTracker;this.players[i].ui=ui;for(var j=0;j<lsPlayers[i].locked.length;j++){var tiles=lsPlayers[i].locked[j].map(t=>new GameTile(null,t.values));this.players[i].lockClaim(tiles);lsPlayers[i].locked[j].forEach(t=>publiclyVisible.push(t.values.tile))}
...
for(var j=0;j<this.players[i].locked.length;j++){for(var k=0;k<this.players[i].locked[j].length;k++){var tile=new GameTile(null,this.players[i].locked[j][k].values);tile.lock(j+1);el.appendChild(tile)}}
for(var j=0;j<this.players[i].tiles.length;j++){el.appendChild(create(-1))}}
this.players[i].bonus=lsPlayers[i].bonus}
if(game.discards.length<1){game.discard=new GameTile(null,game.discard)}
for(var i=0;i<game.discards.length;i++){let tile=game.discards[i].values.tile;let discard=create(tile);discard.mark(`discard`);document.querySelector(`.discards`).appendChild(discard);if(i===game.discards.length-1){game.discard=discard}
...
function offerChoice(){if(!noSleep.enabled)noSleep.enable();const options=[{description:"There are currently two modes of play on offer:"},{label:"I'd like to play some mahjong!",value:"play"},{label:"I just want to watch the bots play",value:"watch"},{description:"Alternatively, you can modify the game settings:",align:"center",},{label:"Change settings",value:"settings",back:!0},{label:"Change theming",value:"theming",back:!0},{description:"(you can also open the settings during play)",align:"center",},];options.fixed=!0;modal.choiceInput("Welcome! What would you like to do?",options,(result)=>{config.BOT_PLAY=result==="watch";if(result==="watch")config.FORCE_OPEN_BOT_PLAY=!0;if(result==="settings")return modal.pickPlaySettings();if(result==="theming")return modal.pickTheming();play()})}})()})()
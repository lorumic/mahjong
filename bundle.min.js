...
addFooter(panel,modalLabel="OK",resolve=()=>{},botDismissible){let ok=document.createElement("button");ok.id="okButton";ok.textContent=modalLabel;ok.addEventListener("click",()=>{this.close([{object:document,evntName:"focus",handler:panel.gainFocus},]);resolve();if(modalLabel==="Back to the scores"||modalLabel==="OK")return;try{document.getElementById("okButton").click()}catch(_){}});panel.appendChild(ok);if(config.BOT_PLAY&&botDismissible){let dismiss=()=>ok.click();setTimeout(()=>dismiss(),config.HAND_INTERVAL);panel.addEventListener("click",()=>(dismiss=()=>{}))}
...
recordScores(scores){scores.forEach((score,b)=>{let d=this.playerbanks[b].dataset;let scoreUnpadded=d.score.replace(/[^0-9.]/g,'');if(!scoreUnpadded)d.score=Game.getPaddedDataScore(0);d.score=Game.getPaddedDataScore(parseFloat(scoreUnpadded)+score)})}
...
this.resume=()=>{}}
static getPaddedDataScore(score){var scoreStr=`${score}`;var len=scoreStr.length;if(len===6)return scoreStr;var pad="‏‏‎ ‎".repeat(2*(6-len));return `${pad}${scoreStr}`}
...
serializedGame.game.currentPlayerId=0;serializedGame.winds=[];for(var i=0;i<4;i++){serializedGame.winds.push(Array.from(document.querySelector(".windicator").children[i].classList).filter(cl=>cl!=="e"&&cl!=="player-wind")[0])}
...
rotateWinds(rules,game.wind,game.windOfTheRound,game.hand,game.draws);for(var i=0;i<4;i++){publiclyVisible.forEach(t=>this.players[i].tracker.seen(t));var handcount=document.querySelector(".hand-counter");handcount.innerHTML=`round ${1+game.windOfTheRound}<br>hand ${game.hand}`;if(game.draws){handcount.innerHTML+=`<br>rtr ${game.draws}`}
var windEl=document.querySelector(".windicator").children[i];windEl.classList.remove("lc","tc","rc","bc");windEl.classList.add(localStorageGame.winds[i])}
if(document.querySelector(".countdown-bar")==null){var bar=document.createElement(`div`);bar.classList.add(`countdown-bar`,"active");document.querySelector(`.discards`).prepend(bar);this.players[0].ui.bar=bar}
return this.newGame(game)}else return this.newGame()}}(function(){var existingGame=localStorage.getItem("mahjongGame");if(existingGame)promptResume();else if(config.PLAY_IMMEDIATELY)play();else offerChoice();function promptResume(){if(noSleep&&!noSleep.enabled)noSleep.enable();modal.choiceInput("Resume previous game?",[{label:`Sure`,value:`yes`},{label:`No, take me to main menu`,value:`no`},],result=>{if(result===`yes`)play(!0);else offerChoice()})}
function play(resume=!1){if(noSleep&&!noSleep.enabled)noSleep.enable();let manager=new GameManager();let game=resume?manager.createFromLocalStorage():manager.newGame();game.startGame(()=>{document.body.classList.add("finished");let gameui=game.players.find((p)=>p.ui).ui;config.flushLog();return modal.showFinalScores(gameui,game.rules,game.scoreHistory,()=>{document.body.classList.remove("finished");rotateWinds.reset();offerChoice()})},resume)}
function offerChoice(){if(noSleep&&!noSleep.enabled)noSleep.enable();const options=[{description:"There are currently two modes of play on offer:"},{label:"I'd like to play some mahjong!",value:"play"},{label:"I just want to watch the bots play",value:"watch"},{description:"Alternatively, you can modify the game settings:",align:"center",},{label:"Change settings",value:"settings",back:!0},{description:"(you can also open the settings during play)",align:"center",},];options.fixed=!0;modal.choiceInput("Welcome! What would you like to do?",options,(result)=>{config.BOT_PLAY=result==="watch";if(result==="watch")config.FORCE_OPEN_BOT_PLAY=!0;if(result==="settings")return modal.pickPlaySettings();if(result==="theming")return modal.pickTheming();play()})}})();for(var i=0;i<4;i++){document.getElementById(i).setAttribute("data-score",Game.getPaddedDataScore(""))}})()